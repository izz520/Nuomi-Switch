# Nuomi Switch 产品需求文档 (PRD)

## 1. 产品概述

### 1.1 产品定位
Nuomi Switch 是一款轻量级的本地桌面应用程序，专注于 Codex 账号的管理和切换。它为用户提供了一个统一的界面来管理多个 Codex 账号（OAuth 和 API Key 类型），实现账号快速切换、配额查看、会话管理等核心功能。
...
- Windows: `%APPDATA%\nuomi-switch`
- Linux: `~/.local/share/nuomi-switch`
...
```mermaid
flowchart TD
    Start["启动 Nuomi Switch"] --> Detect["检测本地 Codex 路径与 auth.json"]
